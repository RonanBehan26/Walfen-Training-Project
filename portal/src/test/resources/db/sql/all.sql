delete from company;
delete from user;

insert into company (id, name, city) values (1, 'Company 1', 'Krakow');
insert into company (id, name, city) values (2, 'Company 2', 'Cork');
insert into company (id, name, city) values (3, 'Company 3', 'Madrid');

insert into user (id, firstName, lastName) values (1, 'John', 'Burke');
insert into user (id, firstName, lastName) values (2, 'Pat', 'Glen');
insert into user (id, firstName, lastName) values (3, 'Jane', 'Doe');